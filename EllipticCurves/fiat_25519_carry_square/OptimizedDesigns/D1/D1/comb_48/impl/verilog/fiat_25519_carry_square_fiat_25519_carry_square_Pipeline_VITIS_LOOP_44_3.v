// ==============================================================
// Generated by Vitis HLS v2023.1.1
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2023 Advanced Micro Devices, Inc. All Rights Reserved.
// ==============================================================

`timescale 1 ns / 1 ps 

module fiat_25519_carry_square_fiat_25519_carry_square_Pipeline_VITIS_LOOP_44_3 (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_idle,
        ap_ready,
        arr_8_reload,
        arr_7_reload,
        arr_6_reload,
        arr_5_reload,
        arr_4_reload,
        arr_3_reload,
        arg1_r_3_reload,
        arg1_r_4_reload,
        arg1_r_5_reload,
        arg1_r_6_reload,
        arg1_r_7_reload,
        zext_ln50,
        arg1_r_2_reload,
        arr_25_out,
        arr_25_out_ap_vld,
        arr_24_out,
        arr_24_out_ap_vld,
        arr_23_out,
        arr_23_out_ap_vld,
        arr_22_out,
        arr_22_out_ap_vld,
        arr_21_out,
        arr_21_out_ap_vld,
        arr_20_out,
        arr_20_out_ap_vld
);

parameter    ap_ST_fsm_pp0_stage0 = 1'd1;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
output   ap_idle;
output   ap_ready;
input  [63:0] arr_8_reload;
input  [63:0] arr_7_reload;
input  [63:0] arr_6_reload;
input  [63:0] arr_5_reload;
input  [63:0] arr_4_reload;
input  [63:0] arr_3_reload;
input  [31:0] arg1_r_3_reload;
input  [31:0] arg1_r_4_reload;
input  [31:0] arg1_r_5_reload;
input  [31:0] arg1_r_6_reload;
input  [31:0] arg1_r_7_reload;
input  [31:0] zext_ln50;
input  [31:0] arg1_r_2_reload;
output  [63:0] arr_25_out;
output   arr_25_out_ap_vld;
output  [63:0] arr_24_out;
output   arr_24_out_ap_vld;
output  [63:0] arr_23_out;
output   arr_23_out_ap_vld;
output  [63:0] arr_22_out;
output   arr_22_out_ap_vld;
output  [63:0] arr_21_out;
output   arr_21_out_ap_vld;
output  [63:0] arr_20_out;
output   arr_20_out_ap_vld;

reg ap_idle;
reg arr_25_out_ap_vld;
reg arr_24_out_ap_vld;
reg arr_23_out_ap_vld;
reg arr_22_out_ap_vld;
reg arr_21_out_ap_vld;
reg arr_20_out_ap_vld;

(* fsm_encoding = "none" *) reg   [0:0] ap_CS_fsm;
wire    ap_CS_fsm_pp0_stage0;
wire    ap_enable_reg_pp0_iter0;
reg    ap_enable_reg_pp0_iter1;
reg    ap_idle_pp0;
wire    ap_block_state1_pp0_stage0_iter0;
wire    ap_block_state2_pp0_stage0_iter1;
wire    ap_block_pp0_stage0_subdone;
wire   [0:0] icmp_ln44_fu_288_p2;
reg    ap_condition_exit_pp0_iter1_stage0;
wire    ap_loop_exit_ready;
reg    ap_ready_int;
wire    ap_block_pp0_stage0_11001;
wire   [62:0] zext_ln50_cast_fu_246_p1;
reg   [62:0] zext_ln50_cast_reg_620;
reg   [3:0] i_fu_90;
wire   [3:0] add_ln44_fu_492_p2;
wire    ap_loop_init;
wire   [3:0] i_1_load_fu_285_p1;
wire    ap_block_pp0_stage0;
reg   [63:0] arr_1_fu_94;
wire   [63:0] arr_7_fu_436_p2;
reg   [63:0] arr_2_fu_98;
wire   [63:0] arr_fu_374_p2;
reg   [63:0] arr_3_fu_102;
reg   [63:0] arr_4_fu_106;
reg   [63:0] arr_5_fu_110;
reg   [63:0] arr_6_fu_114;
wire    ap_block_pp0_stage0_01001;
wire   [31:0] mul_ln50_1_fu_238_p0;
wire   [31:0] mul_ln50_1_fu_238_p1;
wire   [31:0] mul_ln50_fu_242_p0;
wire   [31:0] mul_ln50_fu_242_p1;
wire   [2:0] trunc_ln29_fu_312_p1;
wire   [2:0] tmp_2_fu_322_p9;
wire   [31:0] tmp_2_fu_322_p10;
wire   [62:0] mul_ln50_1_fu_238_p2;
wire   [63:0] shl_ln1_fu_344_p3;
wire   [63:0] tmp_3_fu_352_p10;
wire   [2:0] tmp_4_fu_386_p8;
wire   [31:0] tmp_4_fu_386_p9;
wire   [62:0] mul_ln50_fu_242_p2;
wire   [63:0] shl_ln50_1_fu_406_p3;
wire   [63:0] tmp_5_fu_414_p10;
reg    ap_done_reg;
wire    ap_continue_int;
reg    ap_done_int;
reg   [0:0] ap_NS_fsm;
wire    ap_enable_pp0;
wire    ap_start_int;
wire   [62:0] mul_ln50_1_fu_238_p10;
wire   [62:0] mul_ln50_fu_242_p10;
reg    ap_condition_327;
reg    ap_condition_331;
reg    ap_condition_334;
reg    ap_condition_337;
reg    ap_condition_340;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_CS_fsm = 1'd1;
#0 ap_enable_reg_pp0_iter1 = 1'b0;
#0 ap_done_reg = 1'b0;
end

fiat_25519_carry_square_mul_32ns_32ns_63_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 32 ),
    .din1_WIDTH( 32 ),
    .dout_WIDTH( 63 ))
mul_32ns_32ns_63_1_1_U37(
    .din0(mul_ln50_1_fu_238_p0),
    .din1(mul_ln50_1_fu_238_p1),
    .dout(mul_ln50_1_fu_238_p2)
);

fiat_25519_carry_square_mul_32ns_32ns_63_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 32 ),
    .din1_WIDTH( 32 ),
    .dout_WIDTH( 63 ))
mul_32ns_32ns_63_1_1_U38(
    .din0(mul_ln50_fu_242_p0),
    .din1(mul_ln50_fu_242_p1),
    .dout(mul_ln50_fu_242_p2)
);

fiat_25519_carry_square_mux_8_3_32_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 32 ),
    .din1_WIDTH( 32 ),
    .din2_WIDTH( 32 ),
    .din3_WIDTH( 32 ),
    .din4_WIDTH( 32 ),
    .din5_WIDTH( 32 ),
    .din6_WIDTH( 32 ),
    .din7_WIDTH( 32 ),
    .din8_WIDTH( 3 ),
    .dout_WIDTH( 32 ))
mux_8_3_32_1_1_U39(
    .din0(32'd0),
    .din1(32'd0),
    .din2(32'd0),
    .din3(arg1_r_3_reload),
    .din4(arg1_r_4_reload),
    .din5(arg1_r_5_reload),
    .din6(arg1_r_6_reload),
    .din7(arg1_r_7_reload),
    .din8(tmp_2_fu_322_p9),
    .dout(tmp_2_fu_322_p10)
);

fiat_25519_carry_square_mux_8_3_64_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 64 ),
    .din1_WIDTH( 64 ),
    .din2_WIDTH( 64 ),
    .din3_WIDTH( 64 ),
    .din4_WIDTH( 64 ),
    .din5_WIDTH( 64 ),
    .din6_WIDTH( 64 ),
    .din7_WIDTH( 64 ),
    .din8_WIDTH( 3 ),
    .dout_WIDTH( 64 ))
mux_8_3_64_1_1_U40(
    .din0(64'd0),
    .din1(64'd0),
    .din2(64'd0),
    .din3(arr_2_fu_98),
    .din4(arr_6_fu_114),
    .din5(arr_5_fu_110),
    .din6(arr_4_fu_106),
    .din7(arr_3_fu_102),
    .din8(trunc_ln29_fu_312_p1),
    .dout(tmp_3_fu_352_p10)
);

fiat_25519_carry_square_mux_7_3_32_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 32 ),
    .din1_WIDTH( 32 ),
    .din2_WIDTH( 32 ),
    .din3_WIDTH( 32 ),
    .din4_WIDTH( 32 ),
    .din5_WIDTH( 32 ),
    .din6_WIDTH( 32 ),
    .din7_WIDTH( 3 ),
    .dout_WIDTH( 32 ))
mux_7_3_32_1_1_U41(
    .din0(32'd0),
    .din1(32'd0),
    .din2(arg1_r_2_reload),
    .din3(arg1_r_3_reload),
    .din4(arg1_r_4_reload),
    .din5(arg1_r_5_reload),
    .din6(arg1_r_6_reload),
    .din7(tmp_4_fu_386_p8),
    .dout(tmp_4_fu_386_p9)
);

fiat_25519_carry_square_mux_8_3_64_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 64 ),
    .din1_WIDTH( 64 ),
    .din2_WIDTH( 64 ),
    .din3_WIDTH( 64 ),
    .din4_WIDTH( 64 ),
    .din5_WIDTH( 64 ),
    .din6_WIDTH( 64 ),
    .din7_WIDTH( 64 ),
    .din8_WIDTH( 3 ),
    .dout_WIDTH( 64 ))
mux_8_3_64_1_1_U42(
    .din0(64'd0),
    .din1(64'd0),
    .din2(64'd0),
    .din3(arr_6_fu_114),
    .din4(arr_5_fu_110),
    .din5(arr_4_fu_106),
    .din6(arr_3_fu_102),
    .din7(arr_1_fu_94),
    .din8(trunc_ln29_fu_312_p1),
    .dout(tmp_5_fu_414_p10)
);

fiat_25519_carry_square_flow_control_loop_pipe_sequential_init flow_control_loop_pipe_sequential_init_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(ap_start),
    .ap_ready(ap_ready),
    .ap_done(ap_done),
    .ap_start_int(ap_start_int),
    .ap_loop_init(ap_loop_init),
    .ap_ready_int(ap_ready_int),
    .ap_loop_exit_ready(ap_condition_exit_pp0_iter1_stage0),
    .ap_loop_exit_done(ap_done_int),
    .ap_continue_int(ap_continue_int),
    .ap_done_int(ap_done_int)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_pp0_stage0;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((ap_continue_int == 1'b1)) begin
            ap_done_reg <= 1'b0;
        end else if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_enable_reg_pp0_iter1 <= 1'b0;
    end else begin
        if ((1'b1 == ap_condition_exit_pp0_iter1_stage0)) begin
            ap_enable_reg_pp0_iter1 <= 1'b0;
        end else if (((1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_enable_reg_pp0_iter1 <= ap_start_int;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_1_fu_94 <= arr_3_reload;
        end else if ((1'b1 == ap_condition_327)) begin
            arr_1_fu_94 <= arr_7_fu_436_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_2_fu_98 <= arr_8_reload;
        end else if ((1'b1 == ap_condition_331)) begin
            arr_2_fu_98 <= arr_fu_374_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_3_fu_102 <= arr_4_reload;
        end else if ((1'b1 == ap_condition_327)) begin
            arr_3_fu_102 <= arr_fu_374_p2;
        end else if ((1'b1 == ap_condition_334)) begin
            arr_3_fu_102 <= arr_7_fu_436_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_4_fu_106 <= arr_5_reload;
        end else if ((1'b1 == ap_condition_337)) begin
            arr_4_fu_106 <= arr_7_fu_436_p2;
        end else if ((1'b1 == ap_condition_334)) begin
            arr_4_fu_106 <= arr_fu_374_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_5_fu_110 <= arr_6_reload;
        end else if ((1'b1 == ap_condition_340)) begin
            arr_5_fu_110 <= arr_7_fu_436_p2;
        end else if ((1'b1 == ap_condition_337)) begin
            arr_5_fu_110 <= arr_fu_374_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            arr_6_fu_114 <= arr_7_reload;
        end else if ((1'b1 == ap_condition_331)) begin
            arr_6_fu_114 <= arr_7_fu_436_p2;
        end else if ((1'b1 == ap_condition_340)) begin
            arr_6_fu_114 <= arr_fu_374_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            i_fu_90 <= 4'd3;
        end else if (((icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1))) begin
            i_fu_90 <= add_ln44_fu_492_p2;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        zext_ln50_cast_reg_620[31 : 0] <= zext_ln50_cast_fu_246_p1[31 : 0];
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_condition_exit_pp0_iter1_stage0 = 1'b1;
    end else begin
        ap_condition_exit_pp0_iter1_stage0 = 1'b0;
    end
end

always @ (*) begin
    if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_done_int = 1'b1;
    end else begin
        ap_done_int = ap_done_reg;
    end
end

always @ (*) begin
    if (((ap_start_int == 1'b0) & (ap_idle_pp0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((ap_enable_reg_pp0_iter1 == 1'b0) & (ap_enable_reg_pp0_iter0 == 1'b0))) begin
        ap_idle_pp0 = 1'b1;
    end else begin
        ap_idle_pp0 = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_ready_int = 1'b1;
    end else begin
        ap_ready_int = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_20_out_ap_vld = 1'b1;
    end else begin
        arr_20_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_21_out_ap_vld = 1'b1;
    end else begin
        arr_21_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_22_out_ap_vld = 1'b1;
    end else begin
        arr_22_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_23_out_ap_vld = 1'b1;
    end else begin
        arr_23_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_24_out_ap_vld = 1'b1;
    end else begin
        arr_24_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln44_fu_288_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        arr_25_out_ap_vld = 1'b1;
    end else begin
        arr_25_out_ap_vld = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_pp0_stage0 : begin
            ap_NS_fsm = ap_ST_fsm_pp0_stage0;
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign add_ln44_fu_492_p2 = (i_fu_90 + 4'd2);

assign ap_CS_fsm_pp0_stage0 = ap_CS_fsm[32'd0];

assign ap_block_pp0_stage0 = ~(1'b1 == 1'b1);

assign ap_block_pp0_stage0_01001 = ~(1'b1 == 1'b1);

assign ap_block_pp0_stage0_11001 = ~(1'b1 == 1'b1);

assign ap_block_pp0_stage0_subdone = ~(1'b1 == 1'b1);

assign ap_block_state1_pp0_stage0_iter0 = ~(1'b1 == 1'b1);

assign ap_block_state2_pp0_stage0_iter1 = ~(1'b1 == 1'b1);

always @ (*) begin
    ap_condition_327 = (~(i_1_load_fu_285_p1 == 4'd6) & ~(i_1_load_fu_285_p1 == 4'd5) & ~(i_1_load_fu_285_p1 == 4'd4) & ~(i_1_load_fu_285_p1 == 4'd3) & (icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1));
end

always @ (*) begin
    ap_condition_331 = ((icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1) & (i_1_load_fu_285_p1 == 4'd3));
end

always @ (*) begin
    ap_condition_334 = ((icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1) & (i_1_load_fu_285_p1 == 4'd6));
end

always @ (*) begin
    ap_condition_337 = ((icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1) & (i_1_load_fu_285_p1 == 4'd5));
end

always @ (*) begin
    ap_condition_340 = ((icmp_ln44_fu_288_p2 == 1'd1) & (ap_enable_reg_pp0_iter1 == 1'b1) & (i_1_load_fu_285_p1 == 4'd4));
end

assign ap_enable_pp0 = (ap_idle_pp0 ^ 1'b1);

assign ap_enable_reg_pp0_iter0 = ap_start_int;

assign ap_loop_exit_ready = ap_condition_exit_pp0_iter1_stage0;

assign arr_20_out = arr_1_fu_94;

assign arr_21_out = arr_3_fu_102;

assign arr_22_out = arr_4_fu_106;

assign arr_23_out = arr_5_fu_110;

assign arr_24_out = arr_6_fu_114;

assign arr_25_out = arr_2_fu_98;

assign arr_7_fu_436_p2 = (shl_ln50_1_fu_406_p3 + tmp_5_fu_414_p10);

assign arr_fu_374_p2 = (shl_ln1_fu_344_p3 + tmp_3_fu_352_p10);

assign i_1_load_fu_285_p1 = i_fu_90;

assign icmp_ln44_fu_288_p2 = ((i_fu_90 < 4'd9) ? 1'b1 : 1'b0);

assign mul_ln50_1_fu_238_p0 = zext_ln50_cast_reg_620;

assign mul_ln50_1_fu_238_p1 = mul_ln50_1_fu_238_p10;

assign mul_ln50_1_fu_238_p10 = tmp_2_fu_322_p10;

assign mul_ln50_fu_242_p0 = zext_ln50_cast_reg_620;

assign mul_ln50_fu_242_p1 = mul_ln50_fu_242_p10;

assign mul_ln50_fu_242_p10 = tmp_4_fu_386_p9;

assign shl_ln1_fu_344_p3 = {{mul_ln50_1_fu_238_p2}, {1'd0}};

assign shl_ln50_1_fu_406_p3 = {{mul_ln50_fu_242_p2}, {1'd0}};

assign tmp_2_fu_322_p9 = (3'd2 - trunc_ln29_fu_312_p1);

assign tmp_4_fu_386_p8 = (3'd1 - trunc_ln29_fu_312_p1);

assign trunc_ln29_fu_312_p1 = i_fu_90[2:0];

assign zext_ln50_cast_fu_246_p1 = zext_ln50;

always @ (posedge ap_clk) begin
    zext_ln50_cast_reg_620[62:32] <= 31'b0000000000000000000000000000000;
end

endmodule //fiat_25519_carry_square_fiat_25519_carry_square_Pipeline_VITIS_LOOP_44_3
