...
void fiat_25519_carry_mul(uint32_t out1[10], uint32_t arg1[10], uint32_t arg2[10])
{
 ...
 #pragma HLS ARRAY_PARTITION variable=out1_w type=complete dim=1
    ...
    for(int i1 = 9; i1 > 0; i1 -= TILE_SIZE_I_1)
    {
#pragma HLS PIPELINE II=1
        for(int i2 = 0; i2 < 10; i2 += TILE_SIZE_J_1)
        {
#pragma HLS PIPELINE II=1
          for (int i = 0; i < TILE_SIZE_I_1; i++)
          {
#pragma HLS UNROLL
        	  for (int j = 0; j < TILE_SIZE_J_1; j++)
			  {
#pragma HLS UNROLL
        		  int c = ((i1-i) % 2 == 1 && (i2+j) % 2 == 1) ? UINT8_C(0x26) : UINT8_C(0x13);
    ...
    for(int i1 = 9; i1 >= 0; i1 -= TILE_SIZE_I_2)
    {
#pragma HLS PIPELINE II=1
        for(int i2 = 9; i2 >= 0; i2 -= TILE_SIZE_J_2)
        {
#pragma HLS PIPELINE II=1
...
#pragma HLS UNROLL
				...
				{
#pragma HLS UNROLL
        	      int c = ((i1-i) %2 == 1 && (i2-j)%2 == 1) ?  UINT8_C(0x2) : UINT8_C(0x1);
